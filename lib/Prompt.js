...
const addRole = () => {
    inquirer.prompt([
        ...
        },
        {
            ...
            choices: getChoices(`SELECT id AS "value", name AS "name" FROM department`, false)
        }
    ])
    .then(data => {
...
const getChoices = (queryString, canBeNull) => {
    ...
    connection.query(queryString, (err, result) => {
        if (err) throw err;
        // add each of the rows from the results to the choices array
        result.forEach( row => choices.push({ name: row.name, value: row.value }) );
